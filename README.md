...
媒体卡片以单实例窗口显示在触发位置附近，并自动限制在当前显示器工作区内。卡片包含：

- 媒体来源、封面、标题和艺术家；
- 当前播放时间、总时长和播放进度；
- 上一首、播放/暂停、下一首；
- 上一/下一媒体会话；
- 点击进度条跳转，或拖动进度条预览并在松开时提交跳转。

按 `Esc` 或点击卡片外区域可关闭窗口。卡片打开时使用短促的淡入位移动画，关闭时使用淡出位移动画；关闭动画期间仍会拦截卡片外点击，且卡片控件不再提交新的操作。播放器没有提供封面、时间线或某项控制能力时，卡片会隐藏或禁用对应内容，不影响其他可用功能。

当左键单击配置为“打开媒体卡片”时，插件需要与左键双击动作消歧：左键双击为“无操作”时立即打开；存在左键双击动作时，等待时间取 Windows 系统双击时间与 300ms 的较小值。在该时间内完成双击会优先执行双击动作并取消待打开的卡片。
