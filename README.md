...
- 状态图标区和标题区的左键单击、左键双击可分别配置；右键单击保持整项配置：
  - 无操作
  - 打开媒体卡片
  - 播放/暂停
  - 上一首
...
- 默认图标区左键单击播放/暂停，标题区左键单击打开媒体卡片；左键双击均无操作，滚轮切换媒体会话，右键无操作。
- 支持深色/浅色任务栏和 DPI 缩放。
...
| 状态图标区左键双击 | 无操作 |
| 标题区左键双击 | 无操作 |
| 右键单击显示项 | 无操作，保留 TrafficMonitor 默认菜单 |
| 滚轮向上 | 多媒体会话时切换到上一会话 |
| 滚轮向下 | 多媒体会话时切换到下一会话 |

状态图标关闭后，整个显示项均使用标题区的左键设置。从旧版升级且尚未保存分区绑定时，原有左键动作保留为标题区绑定，状态图标区使用新的默认绑定。默认滚轮动作仍为“上一/下一媒体会话”。如果改为“上一首/下一首”，则滚轮向上为上一首、向下为下一首；如果改为“增减系统音量”，则滚轮向上增大系统主音量、向下减小系统主音量。

## 媒体卡片

媒体卡片用于集中查看和控制当前媒体，显示媒体来源、封面、标题、艺术家和播放进度，并提供切换曲目、播放/暂停、切换媒体会话和调整播放位置等操作。

点击卡片外区域或按 `Esc` 即可关闭。卡片只占自身显示区域，外部点击会直接作用于实际目标，不会覆盖桌面、任务栏或屏幕边缘；打开和关闭保留简短动画，并改善自动隐藏任务栏的兼容性。播放器未提供封面、时间线或某项控制能力时，卡片会自动隐藏或禁用相应内容，其他功能仍可正常使用。

同一区域同时配置左键单击和左键双击动作时，双击优先执行，不会额外触发单击动作；因此该区域的单击会有短暂确认延迟。图标区与标题区互不影响。
...
状态图标区和标题区可分别配置左键单击、左键双击动作；右键单击和滚轮对整个显示项生效。各左键动作可设置为：
...
状态图标关闭后，整个显示项使用标题区的左键绑定。某一区域配置左键双击动作后，该区域的左键单击会有短暂确认延迟；另一分区不受影响。
