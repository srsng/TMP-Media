...
> **后续变更（2026-07-26）：** 本文记录 Phase 5.5 的原始实施方案。Phase 6 已将 `MT_WHEEL_UP` / `MT_WHEEL_DOWN` 固定为上一/下一媒体会话循环切换，并从选项页移除两个滚轮动作下拉框。`wheel_up` / `wheel_down` INI 字段仅为旧配置兼容保留，不再参与事件执行。本文其余左键、双击、右键、显示设置和状态图标设计继续有效；涉及“滚轮自由动作映射”的内容均以 `PLAN.md` Phase 6 和 `DESIGN.md` 当前设计为准。
