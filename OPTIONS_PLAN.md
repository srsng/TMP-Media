...
- [ ] `ExecuteControlAction()` 继续只在 MTA 线程调用 GSMTC；现有播放/暂停、下一首实现不改变。
...
6. 右键下一首：执行下一首且不打开 TrafficMonitor 菜单。
7. 滚轮向上/向下分别映射不同已有动作。
8. 所有触发器无操作：插件不提交任何媒体控制请求。
9. 重启 PluginTester：全部映射从 INI 正确恢复。
