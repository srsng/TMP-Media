...
- 在任务栏显示当前播放媒体的标题。
- 标题显示项始终独占完整双行，不与 TrafficMonitor 的其他显示项共用一列。
...
- 歌词显示项也独占完整双行；标题项和歌词项同时启用时分别占据一列。
...
- 将左键单击、左键双击、右键单击、滚轮向上、滚轮向下分别映射到“无操作 / 播放或暂停 / 上一首 / 下一首”。
...
- `CTrafficMonitorMediaItem::IsDoubleLineExclusive()` 返回 `1`，标题项始终独占双行，不提供单双行开关。
...
- TrafficMonitor 非水平排列模式下，标题项独占一列双行，其他显示项不会出现在同一列。
...
API 可行性已经确认：`ITMPlugin::GetItem(index)` 允许同一插件 DLL 连续返回多个 `IPluginItem`。TrafficMonitor 会按每个显示项唯一的 `GetItemId()` 分别保存启用状态和排序，因此可以在现有标题项之外提供第二个歌词项。
...
- 歌词项的 `IsDoubleLineExclusive()` 返回 `1`，始终独占另一列双行；不与标题项共享一列。
...
- 标题项和歌词项可分别勾选、取消和排序。
- 两项同时启用时各占一列完整双行；只启用其中一项时另一项不占空间。
- 能显示当前歌词行或明确显示“无歌词”。
- 长歌词不破坏任务栏布局。
